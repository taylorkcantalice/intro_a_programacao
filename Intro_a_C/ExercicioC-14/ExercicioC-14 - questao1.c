...
int main(void){
    ...
    for(i = 0; i < num_Discentes; i++){
        media_Inicial = (discente[i].nota1 + discente[i].nota2 + discente[i].nota3) / 3;
        if(media_Inicial >= 4 && media_Inicial < 7){
            printf("\nQual a NOTA DE RECUPERACAO do discente %s (MEDIA ATUAL = %.1lf): ", discente[i].nome_Discente, media_Inicial);
    ...
    for(i = 0; i < num_Discentes; i++){
        ...
        if(media_Inicial >= 4 && media_Inicial < 7){
            media_Final = ((6*media_Inicial) + (4*discente[i].nota_de_recuperacao_Discente))/10;
