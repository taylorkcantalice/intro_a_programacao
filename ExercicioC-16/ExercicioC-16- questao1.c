...
void CarregarDados(){
    ...
    while(num_Discentes < MAX_ALUNOS){
        qLidos = fread(&discente[num_Discentes], sizeof(discente[0]), MAX_ALUNOS - num_Discentes, fp);
        printf("Foram lidos %d discentes.\n", qLidos);
        num_Discentes += qLidos;
        if (feof(fp)){
            break;
        }
    }
    ...
    if (fclose(fp) == 0){
        printf("Dados carregados com sucesso (%d discentes).\n", qLidos);
    }
}
...
void SalvarDados(){
    ...
    total = 0;
    while(total < num_Discentes){
        i = fwrite(&discente[total], sizeof(discente[0]), num_Discentes - total, fp);
        total += i;
    }

    if (fclose(fp) == 0){
        printf("Dados salvos com sucesso (%d discentes)\n", total);
    }
}
